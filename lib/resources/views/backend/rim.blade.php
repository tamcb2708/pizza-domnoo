...
@section('tit', 'Viền bánh')
@section('master')
    <!-- Begin Page Content -->
    <div class="container-fluid">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h2 class="add-domnoo">Danh Sách Viền Của Pizza</h2>
            </div>
            <?php
        ...
        <div class="table-responsive">
            <a href="{{ asset('admin/rim/add-rim') }}" class="btn btn-primary">Thêm Tùy Chọn</a>
            <table class="table table-striped b-t b-light">
                ...
                <tbody class="tbody-domnoo">
                    ...
                    <tr class="domnoo-item-1">
                        <td></td>
                        <td class="domnoo-item-detail">{{ $item->ri_name }}</td>
                        <td class="domnoo-item-detail">{{ number_format($item->ri_price, 0, ',', '.') }}</td>
                            </span></td>
                            <td class="domnoo-item-detail">{{ date('d/m/Y H:i', strtotime($item->created_at)) }}</td>
                            <td>
                                <a href="{{ asset('admin/rim/edit/' . $item->ri_id) }}"class="btn btn-warning domnoo-action-2">
                                    <span style="font-family: wingdings; font-size: 200%;">&#252;</span>
                                    Xem Thông Tin Hoặc Sửa
                                </a>
                                <hr>
                                <a href="{{ asset('admin/rim/delete/' . $item->ri_id) }}"
